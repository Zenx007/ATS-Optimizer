...
export class PdfService {
  ...
  compactPdfLikeHtml(html: string): string {
    ...
    const $ = load(html);
    const isPdfLike = $('.t').length > 0 && $('.pf').length > 0;
    if (!isPdfLike) {
      return this.formatHtmlForStorage(html);
    }

    // Normalize pdf2htmlEX output to semantic HTML only (without bulky CSS/base64 assets).
    return this.toLlmFriendlyResumeHtml(html);
  }

  toLlmFriendlyResumeHtml(html: string): string {
    if (!html) {
      return html;
    }

    const $ = load(html);
    $('style, link[rel="stylesheet"], script, noscript, iframe, object, embed').remove();
    $('#sidebar, #outline, .loading-indicator').remove();
    $('.pi').remove();
    this.handlePageContainer($);

    const pdfTextNodes = $('.t').toArray();
    const rawLines = pdfTextNodes.length
      ? pdfTextNodes.map((node) => this.extractReadableTextFromPdfNode(node))
      : (($('body').text() || $.root().text()) as string).split(/\n+/);
    const normalizedLines = this.normalizeExtractedLines(rawLines);

    if (!normalizedLines.length) {
      return this.formatHtmlForStorage(html);
    }

    const bodyLines = normalizedLines.map(
    ...
    return this.formatHtmlForStorage(compactHtml);
  }
  ...
  private normalizeExtractedLines(rawLines: string[]): string[] {
    const lines: string[] = [];
  ...
  private formatHtmlForStorage(html: string): string {
    if (!html) {
      return html;
    }
